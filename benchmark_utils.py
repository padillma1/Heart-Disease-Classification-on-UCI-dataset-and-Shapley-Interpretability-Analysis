...
def evaluate_model_on_local_and_pooled_tests(
    ...
    # Calculate the weights for each client
    weights = {client: num_samples[client] / total_samples for client in num_samples}    
    print(perf_dict)
    ...
    #average_accuracy = np.mean([v for v in perf_dict.values()])
    #print("Average Accuracy:", average_accuracy)
    
    # Compute weighted average accuracy
    weighted_average_accuracy = np.sum([weights[client] * v for client, v in perf_dict.items()])

    # Compute weighted standard deviation
    weighted_std_dev = np.sqrt(np.sum([weights[client] * ((v - weighted_average_accuracy) ** 2) for client, v in perf_dict.items()]))

    print("Weighted Average Accuracy: {:.3f} +/- {:.4f}".format(weighted_average_accuracy, weighted_std_dev))
